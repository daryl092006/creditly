...
export default function AdminRepaymentTable({
    ...
    return (
        <>
            {rows.length === 0 ? (
                <div className="py-24 flex flex-col items-center justify-center gap-4">
                    <div className="w-16 h-16 rounded-2xl bg-slate-900 border border-white/5 flex items-center justify-center">
                        <svg className="w-8 h-8 text-slate-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.407 2.67 1a2.4 2.4 0 01.4 0M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.407-2.67-1a2.4 2.4 0 01-.4 0M12 12V4a2 2 0 00-2-2H4a2 2 0 00-2 2v10a2 2 0 002 2h2m2-4h12a2 2 0 012 2v10a2 2 0 01-2 2H6a2 2 0 01-2-2V7.242" /></svg>
                    </div>
                    <p className="text-sm font-black text-slate-600 uppercase tracking-widest">Aucun remboursement en attente</p>
                    <p className="text-xs text-slate-700 italic">Les preuves de paiement soumises apparaîtront ici</p>
                </div>
            ) : (
            ...
            {/* Tous les modaux via Portal → plein écran garanti */}
            {mounted && createPortal(
                <>
                    ...
                    {validationPreview && (
                        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(2,6,23,0.97)', backdropFilter: 'blur(16px)' }}>
                            <div className="w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl animate-scale-in" style={{ background: 'linear-gradient(145deg, #0f172a 0%, #1e293b 100%)', border: '1px solid rgba(255,255,255,0.1)' }}>
                                <div className="px-8 pt-8 pb-6 border-b border-white/10">
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <div>
                                            <h3 className="text-xl font-black text-white uppercase tracking-tight">Confirmer la Validation</h3>
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{validationPreview.user.split('(')[0].trim()}</p>
                                        </div>
                                    </div>
                                </div>
                                <div className="px-8 py-6 space-y-5">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="rounded-2xl p-5" style={{ background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)' }}>
                                            <p className="text-[9px] font-black text-emerald-400/70 uppercase tracking-widest mb-1">Montant validé</p>
                                            <p className="text-2xl font-black text-emerald-400 tabular-nums leading-none">{validationPreview.amount.toLocaleString('fr-FR')}</p>
                                            <p className="text-[10px] font-bold text-emerald-400/50 mt-1">FCFA</p>
                                        </div>
                                        <div className="rounded-2xl p-5" style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)' }}>
                                            <p className="text-[9px] font-black text-red-400/70 uppercase tracking-widest mb-1">Dette totale</p>
                                            <p className="text-2xl font-black text-red-400 tabular-nums leading-none">{validationPreview.loanTotalDue.toLocaleString('fr-FR')}</p>
                                            <p className="text-[10px] font-bold text-red-400/50 mt-1">FCFA</p>
                                        </div>
                                    </div>
                                    <div className="rounded-2xl p-5" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
                                        <div className="flex justify-between items-center mb-4">
                                            <p className="text-xs font-black text-slate-300 uppercase tracking-wider">Après validation</p>
                                            {validationPreview.amount >= validationPreview.loanTotalDue && !validationPreview.isExtension ? (
                                                <span className="px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest" style={{ background: 'rgba(16,185,129,0.2)', color: '#34d399', border: '1px solid rgba(16,185,129,0.3)' }}>✓ Soldé</span>
                                            ) : (
                                                <span className="px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest" style={{ background: 'rgba(59,130,246,0.2)', color: '#60a5fa', border: '1px solid rgba(59,130,246,0.3)' }}>Partiel</span>
                                            )}
                                        </div>
                                        <div className="flex items-baseline justify-between">
                                            <span className="text-sm font-bold text-slate-400">Nouveau reste :</span>
                                            <span className={`text-3xl font-black tabular-nums ${validationPreview.amount >= validationPreview.loanTotalDue ? 'text-emerald-400' : 'text-white'}`}>
                                                {Math.max(0, validationPreview.loanTotalDue - validationPreview.amount).toLocaleString('fr-FR')} <span className="text-sm font-bold text-slate-500">FCFA</span>
                                            </span>
                                        </div>
                                    </div>
                                    {validationPreview.amount > (validationPreview.loanTotalDue + 50) && (
                                        <div className="rounded-2xl p-4 flex items-center gap-3" style={{ background: 'rgba(249,115,22,0.08)', border: '1px solid rgba(249,115,22,0.25)' }}>
                                            <div className="w-2 h-2 rounded-full bg-orange-500 animate-pulse flex-shrink-0" />
                                            <p className="text-xs font-black text-orange-400 uppercase tracking-wide">Surplus de {(validationPreview.amount - validationPreview.loanTotalDue).toLocaleString('fr-FR')} FCFA détecté</p>
                                        </div>
                                    )}
                                </div>
                                <div className="px-8 pb-8 flex flex-col gap-3">
                                    <button
                                        ...
                                        className="w-full py-4 rounded-2xl font-black text-sm uppercase tracking-widest text-white transition-all active:scale-95 disabled:opacity-60"
                                    ...
                                    <button
                                        ...
                                        className="w-full py-3 rounded-2xl font-black text-xs uppercase tracking-widest text-slate-500 hover:text-white transition-colors"
