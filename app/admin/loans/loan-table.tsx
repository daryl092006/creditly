        ...
        <div className="admin-loan-table-wrapper">
            {rows.length === 0 ? (
                <div className="py-24 flex flex-col items-center justify-center gap-4">
                    <div className="w-16 h-16 rounded-2xl bg-slate-900 border border-white/5 flex items-center justify-center text-slate-700">
                    ...
                    <p className="text-sm font-black text-slate-600 uppercase tracking-widest">Aucun prêt trouvé</p>
                    <p className="text-xs text-slate-700 italic">La liste des demandes de prêt est vide</p>
                </div>
            ) : (
                ...
                <>
                    ...
                    >
                        {viewWaiver && (
                            <div className="space-y-6">
                                <div id="admin-printable-waiver-modal" className="w-full mt-4 bg-white border border-slate-200 rounded-2xl p-6 space-y-6 text-left font-sans animate-fade-in max-h-[60vh] overflow-y-auto custom-scrollbar shadow-inner text-slate-700">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 no-print">
                                    {!['pending', 'rejected'].includes(viewWaiver.status) && (
                                        <>
                                            ...
                                            >
                                                <Download size={18} />
                                                {downloadingId === viewWaiver.id ? 'Génération...' : 'Télécharger PDF'}
                                            </button>
                                        </>
                                    )}
                                </div>
                    ...
                    {viewWaiver && (
                        <div className="print-only-container text-black bg-white p-12 font-serif relative" id="admin-printable-waiver">
                            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 -rotate-45 pointer-events-none opacity-[0.1] select-none text-[150px] font-black tracking-tighter z-0">CERTIFIÉ</div>
                            <div className="max-w-[800px] mx-auto relative z-10">
                                ...
                                <div className="space-y-8 text-sm leading-relaxed text-black">
                                    <p className="text-[15px]">Je soussigné(e), Monsieur/Madame <strong>{viewWaiver.user.split('(')[0]}</strong>, demeurant au <strong>{viewWaiver.borrower_address || '________________'}</strong>, ville de <strong>{viewWaiver.borrower_city || '________________'}</strong>, titulaire de la pièce d'identité N° <strong>{viewWaiver.borrower_id_details || '________________'}</strong>.</p>
                                    <p className="text-base text-justify">Reconnais par la présente, avoir contracté auprès de la plateforme <strong>Creditly</strong>, un prêt d'un montant de <strong>{viewWaiver.amount.toLocaleString('fr-FR')} FCFA</strong>, majoré de frais de dossier, soit un total de :</p>
                                    <div className="text-center p-8 bg-gray-50 border-4 border-double border-black">
                                        <div className="font-black text-4xl italic tracking-tighter mb-2">{(viewWaiver.amount + (viewWaiver.service_fee || 0)).toLocaleString('fr-FR')} FCFA</div>
                                        <div className="text-[10px] font-black uppercase text-gray-500 border-t border-gray-200 pt-2 inline-block px-10">{numberToFrench(viewWaiver.amount + (viewWaiver.service_fee || 0)).toUpperCase()} FRANCS CFA</div>
                                    </div>
                                    <p className="text-base">Je m'engage formellement à rembourser l'intégralité de cette somme au profit de <strong>Creditly</strong> au plus tard le :</p>
                                    <div className="text-center py-4 bg-gray-100 border-x-4 border-black font-black text-2xl underline decoration-double">
                                        {viewWaiver.due_date ? new Date(viewWaiver.due_date).toLocaleDateString('fr-FR') : '________________'}
                                    </div>
                                    <div className="grid grid-cols-2 gap-12 mt-20 pt-10 border-t-2 border-black">
                                        <div>
                                            <p className="font-black underline mb-14 uppercase text-[10px] tracking-widest">Le Débiteur (Signature)</p>
                                            <p className="italic font-serif text-xl font-bold text-blue-900 leading-none">{viewWaiver.user.split('(')[0]}</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="font-black underline mb-14 uppercase text-[10px] tracking-widest">Le Créancier (Creditly)</p>
                                            <p className="text-[9px] font-black text-blue-900/50 uppercase italic">Certification Automatique</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <style jsx global>{`
                        @media print {
                            ...
                            .print-only-container { display: block !important; position: absolute !important; left: 0 !important; top: 0 !important; width: 100% !important; background: white !important; margin: 0 !important; padding: 20px !important; z-index: 9999999 !important; }
                            .print-only-container * { color: black !important; background-color: transparent !important; border-color: black !important; }
