...
import { TrashCan, User, CheckmarkFilled, Misuse, InformationFilled, ChevronDown, Rocket, ChartPie } from '@carbon/icons-react'
...
export default function UserManagementTable({ rows, currentUserRoles, initialShareholders }: {
    ...
    return (
        <div className="admin-loan-table-wrapper space-y-6">
            <AdminTableFilters
            ...
            {rows.length === 0 ? (
                <div className="glass-panel p-20 text-center bg-slate-900/50 border-slate-800">
                    <div className="w-16 h-16 rounded-2xl bg-slate-900 border border-white/5 mx-auto flex items-center justify-center text-slate-700 mb-6">
                        <User size={32} />
                    </div>
                    <p className="text-sm font-black text-slate-600 uppercase tracking-widest">Aucun utilisateur trouvé</p>
                    <p className="text-xs text-slate-700 italic mt-2">La base de données des utilisateurs est vide</p>
                </div>
            ) : (
                <>
