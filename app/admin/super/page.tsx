        ...
        <div className="py-10 md:py-16 animate-fade-in">
            ...
            <section>
                <h3 className="text-xl font-black text-white tracking-tighter uppercase italic flex items-center gap-3 mb-6">
                    ...
                    Disponibilités Mensuelles (Quotas Globaux)
                </h3>
                <div className="glass-panel p-8 bg-slate-900/50 border-slate-800 relative overflow-hidden group">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-8 relative z-10">
                        {[5000, 10000, 25000, 50000, 100000].map(amt => {
                            ...
                            return (
                                <div key={amt} className="space-y-4">
                                    <div className="flex justify-between items-baseline">
                                        <p className="text-xl font-black text-white italic tracking-tighter">{amt.toLocaleString()} <span className="text-[10px] not-italic text-slate-700 uppercase">F</span></p>
                                        <span className={`text-[10px] font-black uppercase tracking-widest ${remaining === 0 ? 'text-red-500 animate-pulse' : 'text-emerald-500'}`}>
                                            {remaining} / {q?.limit || 0}
                                        </span>
                                    </div>
                                    <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                                        <div
                                            className={`h-full transition-all duration-1000 ${remaining === 0 ? 'bg-red-500' : 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.3)]'}`}
                                            style={{ width: `${percentage}%` }}
                                        ></div>
                                    </div>
                                    <p className="text-[9px] font-black text-slate-600 uppercase tracking-[0.2em] italic">
                                        {remaining === 0 ? 'PLAFOND ATTEINT' : 'CAPACITÉ RESTANTE'}
                                    </p>
                                </div>
                            )
                        })}
