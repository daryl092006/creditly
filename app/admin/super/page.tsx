...
import { User, Currency, Document, ChevronRight, Filter } from '@carbon/icons-react'
            ...
            <div className="main-container space-y-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {[
                        ...
                        { label: 'Remboursements Mensuels', value: monthlyRepaymentVolume, color: 'text-blue-400', sub: `Paiements validés`, icon: <Currency /> },
                        { label: 'Volume Prêts Mensuel', value: monthlyLoanVolume, color: 'text-white', sub: 'Demandes accordées', icon: <Document /> },
                        { label: 'Total Clients', value: totalUsers, color: 'text-slate-400', sub: 'Comptes actifs', icon: <User /> }
                    ].map((kpi, i) => (
                        <div key={i} className="glass-panel p-8 group relative overflow-hidden bg-slate-900/50 border-slate-800">
                            ...
                            <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-4 italic leading-none">{kpi.label}</p>
                            <p className={`text-4xl font-black tracking-tighter italic ${kpi.color}`}>
                                {typeof kpi.value === 'number' ? kpi.value.toLocaleString() : kpi.value}
                                {typeof kpi.value === 'number' && kpi.label.includes('Mensuel') && <span className="text-xs not-italic text-slate-700 ml-1">FCFA</span>}
                            </p>
                        ...
                        <section>
                            <div className="flex items-center justify-between mb-6">
                                <h3 className="text-xl font-black text-white tracking-tighter uppercase italic flex items-center gap-3">
                                    ...
                                    Prêts Actifs (Top 5 Urgences)
                                </h3>
                                <Link href="/admin/loans" className="text-[10px] font-black text-blue-500 uppercase tracking-widest italic hover:underline">Voir tout</Link>
                            </div>
                                    ...
                                    <tbody className="divide-y divide-white/5">
                                        {activeLoans?.map(loan => (
                                            <tr key={loan.id} className="hover:bg-white/5 transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-sm font-black text-white italic">{loan.amount.toLocaleString()} FCFA</td>
                                                <td className="px-6 py-4">
                                                    <span className="text-[10px] font-black text-amber-500 uppercase italic">{new Date(loan.due_date).toLocaleDateString()}</span>
                                                </td>
