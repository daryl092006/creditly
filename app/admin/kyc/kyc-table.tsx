...
export default function AdminKycClientTable({ submissions }: {
    ...
    return (
        <>
            {submissions.length === 0 ? (
                <div className="py-24 flex flex-col items-center justify-center gap-4">
                    <div className="w-16 h-16 rounded-2xl bg-slate-900 border border-white/5 flex items-center justify-center">
                        <svg className="w-8 h-8 text-slate-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                    </div>
                    <p className="text-sm font-black text-slate-600 uppercase tracking-widest">Aucun dossier KYC en attente</p>
                    <p className="text-xs text-slate-700 italic">Les nouvelles soumissions apparaîtront ici</p>
                </div>
            ) : (
            ...
            {/* Modaux via Portal → plein écran garanti */}
                        ...
                        {confirmAction?.status === 'rejected' && (
                            <div className="w-full mt-4 space-y-3">
                                <div className="flex flex-wrap gap-2 mb-2">
                                    <button type="button" onClick={() => setRejectionReason("Le nom/prénom sur votre profil ne correspond pas à la pièce d'identité. Veuillez le modifier dans 'Mes Informations'.")} className="px-3 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 text-[9px] font-black uppercase tracking-widest rounded-lg border border-red-500/20 transition-colors text-left">Nom non compatible</button>
                                    <button type="button" onClick={() => setRejectionReason("La pièce d'identité est floue, coupée ou illisible. Veuillez fournir une photo plus nette.")} className="px-3 py-1.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 text-[9px] font-black uppercase tracking-widest rounded-lg border border-amber-500/20 transition-colors text-left">Pièce illisible</button>
                                    <button type="button" onClick={() => setRejectionReason("Le selfie ne permet pas de vous identifier formellement avec votre pièce d'identité.")} className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-[9px] font-black uppercase tracking-widest rounded-lg border border-slate-700 transition-colors text-left">Selfie invalide</button>
                                </div>
                                <textarea value={rejectionReason} onChange={(e) => setRejectionReason(e.target.value)} placeholder="Ex: Document illisible, informations incorrectes..." className="w-full h-32 p-4 bg-black/40 border border-white/10 rounded-2xl text-white text-xs font-bold italic focus:border-red-500/50 outline-none transition-all placeholder:text-slate-700 resize-none" />
                            </div>
                        )}
                    </ConfirmModal>
                    <ConfirmModal
