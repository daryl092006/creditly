...
-- 1. Ajout de la colonne service_fee à abonnements (au cas où ce n'est pas fait)
ALTER TABLE public.abonnements ADD COLUMN IF NOT EXISTS service_fee numeric DEFAULT 500;

-- 2. Ajout des colonnes de snapshot à user_subscriptions
...
-- 3. Backfill des données existantes (on fige les valeurs actuelles pour les anciens abonnements)
...
-- 4. Fonction RPC pour prendre le snapshot à l'activation ou souscription
-- On mettra à jour la logique applicative au lieu d'un trigger pour plus de contrôle.
-- Mais pour l'instant, c'est bon.

-- 5. Mise à jour de la RPC des prêts pour utiliser le snapshot de l'abonnement
