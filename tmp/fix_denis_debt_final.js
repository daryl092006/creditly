...
async function fixDenisDebt() {
    // 1. Get loan
    const loans = await request(`/rest/v1/prets?user_id=eq.02407183-6552-4b79-a72a-d68a29623e98&status=eq.overdue&select=*`);
    if (!loans.length) {
        console.log("No overdue loan found for Denis.");
        return;
    }
    const loan = loans[0];
