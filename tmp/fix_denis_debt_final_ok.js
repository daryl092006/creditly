...
async function fixDenisDebt() {
    ...
    // 1. Get loan
    const loans = await request(`/rest/v1/prets?user_id=eq.${denisId}&or=(status.eq.overdue,status.eq.active)&select=*`);
    if (!loans.length) {
    ...
    const isFullyPaid = newPaid >= principle - 10; // small margin
    ...
    console.log(`Fixed. New Status=${isFullyPaid ? 'paid' : 'active'}`);
}
